...
var Demo = function (options) {
  ...
  var contactDotMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff })
  ...
  function init() {
    ...
    world.addEventListener('postStep', function (evt) {
      for (var label in world.profile) lines[label].append(world.time * 1000, world.profile[label])
    })
    ...
    rf.add(settings, 'particleSize')
      ...
      .onChange(function (size) {
        for (var i = 0; i < visuals.length; i++) {
          if (bodies[i] instanceof CANNON.Particle) visuals[i].scale.set(size, size, size)
        }
      })
    rf.add(settings, 'shadows')
      .onChange(function (shadows) {
        if (shadows) {
          light.castShadow = true
